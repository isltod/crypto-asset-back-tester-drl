"""
flare.backtest.test_swing_fine_grid

Mode 2.1 (FLARE-Swing-Pure, 24h)에 대해
익절 TP [12, 16, 20, 24]% x 손절 SL [3, 4, 5]%
총 12개 조합 정밀 그리드 서치 백테스트 및 성과 매트릭스 산출 모듈
"""
...
def main():
    ...
    tp_levels = [12.0, 16.0, 20.0, 24.0]
    sl_levels = [3.0, 4.0, 5.0]
    
    records = []
    
    print("=" * 115)
    print("🔬 [Mode 2.1: FLARE-Swing-Pure] TP [12,16,20,24]% x SL [3,4,5]% (12개 조합) 정밀 그리드 서치")
    print("=" * 115)
    ...
    print("\n" + "=" * 115)
    print("🏆 [전체 12개 조합 상세 랭킹]")
    print("-" * 115)
    sorted_df = res_df.sort_values("return_pct", ascending=False).reset_index(drop=True)
    for i, r in sorted_df.iterrows():
        print(f"{i+1:2d}위: TP +{r['tp']:.0f}% / SL -{r['sl']:.0f}% ➔ 누적수익률: {r['return_pct']:>+6.2f}% | 승률: {r['win_rate']:>4.1f}% | MDD: {r['mdd']:>5.2f}% | 손익비: {r['profit_factor']:>4.2f} | 거래수: {r['total_trades']}회 (TP:{r['tp_count']} SL:{r['sl_count']} TO:{r['timeout_count']})")
    print("=" * 115)
