"""
flare.backtest.test_sniper_grid

Mode 1.1 (FLARE-Sniper-Pure, 4h)에 대해
익절 TP [0.5, 1.0, 2.0, 3.0]% x 손절 SL [0.5, 1.0, 2.0, 3.0]%
총 16개 조합 2D 그리드 서치 백테스트 및 성과 매트릭스 산출 모듈
"""
...
def main():
    ...
    # Mode 1.1 진입 시그널: 펀딩비 하위 10% 이내 + 5분봉 청산 꼬리
    sig_sniper_pure = (
    ...
    sl_levels = [0.5, 1.0, 2.0, 3.0]
    
    records = []
    
    print("=" * 115)
    print("🔬 [Mode 1.1: FLARE-Sniper-Pure (4h)] TP x SL (4x4 = 16개 조합) 2D 그리드 서치 실행")
    print("=" * 115)
    ...
    for i, r in top5.reset_index(drop=True).iterrows():
        print(f"{i+1}위: TP +{r['tp']:.1f}% / SL -{r['sl']:.1f}% ➔ 누적수익률: {r['return_pct']:>+6.2f}% | 승률: {r['win_rate']:>4.1f}% | MDD: {r['mdd']:>5.2f}% | 손익비: {r['profit_factor']:>4.2f} | 거래수: {r['total_trades']}회 (TP:{r['tp_count']} SL:{r['sl_count']} TO:{r['timeout_count']})")
    print("=" * 115)
