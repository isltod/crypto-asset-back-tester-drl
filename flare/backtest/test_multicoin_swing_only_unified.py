"""
flare.backtest.test_multicoin_swing_only_unified

5대 코인 (BTC, ETH, SOL, DOGE, XRP)에 대해
Mode 1.1(스나이퍼)을 완전히 제외하고, Mode 2.1 (스윙 24h) 전용으로만
...
def run_swing_only_multicoin():
    data_dir = Path(__file__).resolve().parent.parent.parent / "data"
    symbols = ["BTCUSDT", "ETHUSDT", "SOLUSDT", "DOGEUSDT", "XRPUSDT"]
    
    print("[*] 5대 코인 4개년 시계열 데이터 로드 중...")
    ...
    print(f"[*] [Mode 2.1 스윙 전용] 4개년 1계좌 1포지션 실전 복리 시뮬레이션 가동...")
    ...
    for current_time in master_timeline:
        ...
        # 2. 무포지션 상태일 때만 5개 코인의 스윙 신호 탐색 (스나이퍼 완전 배제)
        if active_pos is None:
            ...
            for sym in symbols:
                ...
                if current_time in sym_data.index:
                    row = sym_data.loc[current_time]
                    if row["sig_swing"]: # 오직 Mode 2.1 스윙 신호만 진입!
                        c = row["close"]
    ...
    print("\n" + "=" * 115)
    print("🏆 [Mode 2.1 스윙 전용] 5대 코인 1계좌 1포지션 실전 복리 백테스트 성적표 (2021~2024, 4개년)")
