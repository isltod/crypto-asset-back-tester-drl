"""
[실험 54] 진짜 무제한 레버리지 100x (max_leverage=100.0 완전 해제)
- 배경: 이전 PositionManager의 max_leverage 기본값(5.0x) 제한으로 인해 실질 5배 캡에 걸려있었음.
- 이번 실험: default_leverage=100.0, max_leverage=100.0 완전 개방!
- MR 리스크 [4%, 8%, 12%, 16%, 20%, 30%, 40%] x TF [2%, 4%, 6%, 8%, 10%] 35개 조합 전수 스캔
"""
import os
import sys
import numpy as np
...
def run_experiment_54():
    ...
    print("=" * 105)
    print("      [실험 54] 진짜 무제한 레버리지 100x 개방! MR [4%~40%] x TF [2%~10%] 35개 전수 스캔")
    print("=" * 105)

    tf_levels = [0.02, 0.04, 0.06, 0.08, 0.10]
    mr_levels = [0.04, 0.08, 0.12, 0.16, 0.20, 0.30, 0.40]
    results = []
    ...
    for t_risk in tf_levels:
        for m_risk in mr_levels:
            ...
            # PositionManager 내부의 5.0x 캡을 100.0x로 완전 해제!
