"""
[실험 42] 국면별 차등 리스크 2차원 그리드 서치 (2.0% ~ 4.0% 집중 탐색)
- 대상: RADE 공식 표준 프로파일 (TH=0.74, CASH 모드)
- 추세장(BULL_TREND) 리스크: [2.0%, 2.5%, 3.0%, 3.5%, 4.0%]
- 횡보장(RANGE) 리스크:      [2.0%, 2.5%, 3.0%, 3.5%, 4.0%]
- 총 5 x 5 = 25개 조합 전수 백테스트
- 목적: 수익 극대화와 MDD 관리 사이의 최적 국면별 리스크 조합 도출
"""
import os
import sys
import numpy as np
...
def run_experiment_42():
    ...
    print("=" * 105)
    print("      [실험 42] 국면별 차등 리스크 (2.0% ~ 4.0%) 25개 조합 2D 그리드 서치 (TH=0.74, CASH 모드)")
    print("=" * 105)

    risk_levels = [0.020, 0.025, 0.030, 0.035, 0.040]
    grid_results = []
    ...
    print("\n[1. 4개년 총 수익률 매트릭스 (%) - 행: 추세장(TF) 리스크 / 열: 횡보장(MR) 리스크]")
    print(pnl_matrix.round(1))

    # 2. 2D 매트릭스: 최대 낙폭 MDD (%)
    mdd_matrix = df_res.pivot(index="trend_risk", columns="mr_risk", values="mdd_pct")
    print("\n[2. 최대 낙폭 MDD 매트릭스 (%) - 행: 추세장(TF) 리스크 / 열: 횡보장(MR) 리스크]")
    print(mdd_matrix.round(2))

    # 3. 2D 매트릭스: 칼마 비율 (가성비 지표 = 수익률 / MDD)
    calmar_matrix = df_res.pivot(index="trend_risk", columns="mr_risk", values="calmar_ratio")
    print("\n[3. 칼마 비율 (수익률 ÷ MDD) - 행: 추세장(TF) 리스크 / 열: 횡보장(MR) 리스크]")
    print(calmar_matrix.round(2))

    # 4. 2022년 하락장 손익 매트릭스 ($)
    y22_matrix = df_res.pivot(index="trend_risk", columns="mr_risk", values="y2022_pnl")
    print("\n[4. 2022년 크립토 윈터 PnL ($) - 행: 추세장(TF) 리스크 / 열: 횡보장(MR) 리스크]")
    print(y22_matrix.round(1))

    # 최적 조합 TOP 5 출력
    print("\n[5. 칼마 비율(수익/위험 가성비) 기준 TOP 5 최적 조합]")
    top_calmar = df_res.sort_values(by="calmar_ratio", ascending=False).head(5)
    for idx, r in top_calmar.iterrows():
        print(f" * 추세 {r['trend_risk']:.1f}% x 횡보 {r['mr_risk']:.1f}% ──► 수익: +${r['profit_dollars']:,.2f} (+{r['total_return_pct']:.1f}%) | MDD: {r['mdd_pct']:.2f}% | Calmar: {r['calmar_ratio']:.2f} | 2022년: {r['y2022_pnl']:+8.1f}$")

    print("\n[6. 단순 총 수익금 기준 TOP 5 조합]")
    top_pnl = df_res.sort_values(by="profit_dollars", ascending=False).head(5)
