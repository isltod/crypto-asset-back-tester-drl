"""
[실험 44] 황금 차등 리스크 (추세 2.5% x 횡보 4.0%) + 비대칭 BEAR TH=0.80 숏 융합 백테스트
- 모델 1: STANDARD_GOLDEN (현금 관망, 추세 2.5% x 횡보 4.0%, TH=0.74)
- 모델 2: 기존 AGGRESSIVE_SHORT (비대칭 80% 숏, 단일 2.0% 리스크)
- 모델 3: 신규 융합 GOLDEN_SHORT (비대칭 80% 숏 + 추세 2.5% x 횡보 4.0% 리스크)
"""
import os
import sys
import numpy as np
...
def run_experiment_44():
    ...
    print("=" * 105)
    print("      [실험 44] 황금 차등 리스크(2.5% x 4.0%) + 비대칭 80% 숏 융합 백테스트")
    print("=" * 105)

    # 1. 모델 1: STANDARD_GOLDEN (CASH 모드)
    ...
    # 모델 2: 기존 AGGRESSIVE_SHORT (단일 2.0% 숏)
    ...
    # 모델 3: 신규 융합 GOLDEN_SHORT (추세 2.5% x 횡보 4.0% + 비대칭 80% 숏)
    ...
    print(f"{'지표 항목':<22} | {'① STANDARD_GOLDEN (현금관망)':<30} | {'② 기존 AGGRESSIVE_SHORT (2.0%)':<32} | {'③ 신규 융합 GOLDEN_SHORT (2.5%x4.0%)':<35}")
    print("-" * 125)
    ...
    print(f"{'4개년 총 수익금':<22} | {fmt_pnl(res_m1):<30} | {fmt_pnl(res_m2):<32} | {fmt_pnl(res_m3):<35}")
    print(f"{'최대 낙폭 (MDD)':<22} | {fmt_mdd(res_m1):<30} | {fmt_mdd(res_m2):<32} | {fmt_mdd(res_m3):<35}")
    print(f"{'칼마 비율 (수익÷MDD)':<22} | {fmt_cal(res_m1):<30} | {fmt_cal(res_m2):<32} | {fmt_cal(res_m3):<35}")
    print(f"{'손익비 (PF)':<22} | {fmt_pf(res_m1):<30} | {fmt_pf(res_m2):<32} | {fmt_pf(res_m3):<35}")
    print(f"{'전체 승률 (Win Rate)':<22} | {fmt_wr(res_m1):<30} | {fmt_wr(res_m2):<32} | {fmt_wr(res_m3):<35}")
    print(f"{'총 거래 횟수':<22} | {fmt_tr(res_m1):<30} | {fmt_tr(res_m2):<32} | {fmt_tr(res_m3):<35}")
    print("-" * 125)
    ...
    for yr in [2021, 2022, 2023, 2024]:
        ...
        print(f" * {yr}년: ① STANDARD_GOLDEN -> {p1:>+10.2f}$ | ② AGGRESSIVE_SHORT -> {p2:>+10.2f}$ | ③ GOLDEN_SHORT -> {p3:>+10.2f}$")

    # 3. 모델 3의 국면별/포지션별 세부 성과 분해
    print("\n[3. 신규 융합 GOLDEN_SHORT 국면별/방향별 성과 분해]")
    for yr in [2021, 2022, 2023, 2024]:
        sub_yr = t3[t3["year"] == yr]
        print(f"\n--- [ {yr}년 세부 실적 ] ---")
        for eng, side_str in [("TREND_FOLLOWING", "LONG"), ("TREND_FOLLOWING", "SHORT"), ("MEAN_REVERSION", "LONG"), ("MEAN_REVERSION", "SHORT")]:
            sub = sub_yr[(sub_yr["engine"] == eng) & (sub_yr["side"].astype(str).str.contains(side_str))]
            if len(sub) > 0:
                p_sum = sub["pnl"].sum()
                wr = len(sub[sub["pnl"] > 0]) / len(sub) * 100
                print(f" * {eng:<16} ({side_str:<5}): {len(sub):2d}회 | 승률 {wr:5.1f}% | PnL: {p_sum:>+10.2f}$ (건당 {p_sum/len(sub):>+8.2f}$)")
